...
export class ReviewsService {
    ...
    async createReview(userId: number, dto: { expertId: number, sessionId?: number, rating: number, comment?: string }) {
        ...
        if (sessionId) {
            ...
            if (session.userId !== userId) throw new BadRequestException('You can only review sessions you participated in');

            // Check if already reviewed for this session
            const existingReview = await this.reviewRepo.findOne({ where: { sessionId } });
            if (existingReview) throw new BadRequestException('You have already submitted a review for this session');
        }
