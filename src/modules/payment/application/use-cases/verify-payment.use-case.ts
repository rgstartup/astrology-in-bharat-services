...
export class VerifyPaymentUseCase {
  ...
  async execute(dto: VerifyPaymentDto) {
    ...
    try {
      ...
      const isProduct =
        order.notes?.type === 'product' ||
        order.notes?.is_order === true ||
      ...
      if (!isProduct) {
        ...
        await this.orderFacade.markAsPaid(
          order.razorpay_order_id!,
          queryRunner,
        );
        this.logger.log(
          `Product payment verified for order ${order.razorpay_order_id}`,
        );
        await queryRunner.commitTransaction();
        return {
          success: true,
          message: 'Payment successful for product order',
        };
      }
    } catch (error) {
