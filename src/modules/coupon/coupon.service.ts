...
import { BulkAssignCouponDto, UserFilterDto, SpendingPeriod, UserFilterListDto } from './dto/bulk-assign-coupon.dto';
...
export class CouponService {
    ...
    async remove(id: number) {
        ...
        await this.userCouponRepository.delete({ couponId: id });
    ...
    async getAdminStats() {
        ...
        const usedToday = await this.userCouponRepository.count({
            where: {
                isUsed: true,
    ...
    async bulkAssignCoupon(dto: BulkAssignCouponDto) {
        ...
        const existingAssignments = await this.userCouponRepository.find({
            where: {
                couponId: coupon.id,
                userId: In(userIds),
            },
    ...
    async applyCoupon(code: string, userId: number, orderValue?: number, serviceType: string = 'all') {
        ...
        const assignment = await this.userCouponRepository.findOne({
