...
export class GoogleAuthGuard extends AuthGuard('google') {
  ...
  getAuthenticateOptions(context: ExecutionContext) {
    ...
    const role = (typeof query.role === 'string' && query.role) || request.role;
    const referralCode =
  ...
  handleRequest<TUser = any>(
    ...
    this.logger.log(
      `Google Auth handleRequest - url: ${request.url}, err: ${err}, user: ${!!user}, info: ${JSON.stringify(info)}`,
    );
    if (!user) {
    ...
    if (err || !user) {
      ...
      if (rawState) {
        try {
          state = JSON.parse(decodeURIComponent(rawState)) as Record<
            string,
            unknown
          >;
        } catch (_e) {
      ...
      let redirectBase = (state.redirect_uri ||
        state.redirectUrl ||
      ...
      if (
        redirectBase.includes('localhost:3003') ||
