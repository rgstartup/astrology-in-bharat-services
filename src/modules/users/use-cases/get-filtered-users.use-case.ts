...
import {
  Order,
  OrderStatus,
} from '@/modules/commerce/order/entities/order.entity';
...
export class GetFilteredUsersUseCase {
  ...
  private buildBaseQuery(filters: FilterCriteria): SelectQueryBuilder<User> {
    ...
    if (filters.isBlocked !== undefined) {
      query.andWhere('user.is_blocked = :isBlocked', {
  ...
  private applyComplexFilters(
      ...
      query
        .andWhere((qb) => {
          const subQuery = qb
            ...
            .andWhere('session.status = :sessStatus', {
              sessStatus: ChatSessionStatus.COMPLETED,
            })
            .getQuery();
    ...
    // Filter by Spending
    if (
      filters.minSpending !== undefined ||
      filters.maxSpending !== undefined ||
      filters.userType === 'premium' ||
      filters.userType === 'regular'
    ) {
      const now = new Date();
      let sinceDate: Date | null = null;
      if (filters.spendingPeriod && filters.spendingPeriod !== 'all_time') {
        sinceDate = new Date();
        if (filters.spendingPeriod === 'last_month')
          sinceDate.setMonth(now.getMonth() - 1);
        else if (filters.spendingPeriod === 'last_3_months')
          sinceDate.setMonth(now.getMonth() - 3);
        else if (filters.spendingPeriod === 'last_6_months')
          sinceDate.setMonth(now.getMonth() - 6);
      }

      // Subquery for total spending
      const getSpendingSubquery = (qb: SelectQueryBuilder<User>) => {
        const sq = qb
          .subQuery()
          .select('COALESCE(SUM(order.total_amount), 0)')
          .from(Order, 'order')
          .where('order.client_id = user.id')
          .andWhere('order.status = :orderPaid', {
      ...
      if (filters.minSpending !== undefined) {
        query.andWhere(`${getSpendingSubquery(query)} >= :minSpend`, {
          minSpend: filters.minSpending,
        });
      }

      if (filters.maxSpending !== undefined) {
        query.andWhere(`${getSpendingSubquery(query)} <= :maxSpend`, {
          maxSpend: filters.maxSpending,
        });
      }
    }
  }
  ...
  async executeList(filters: FilterCriteria) {
    ...
    const enhancedUsers = await Promise.all(
      users.map(async (u) => {
        ...
        const spendingResult = (await this.dataSource
          .getRepository(Order)
        ...
        return {
          ...
          name: u.name,
          email: u.email,
          totalSpending: parseFloat(spendingResult?.total as string) || 0,
          sessionCount,
