...
export class GetAdminRevenueTrendUseCase {
  ...
  async execute(days: number = 7) {
    ...
    const stats = await this.transactionRepository
      ...
      .addSelect("SUM(CASE WHEN tx.purpose = 'consultation' THEN tx.amount ELSE 0 END)", "consultation")
      .addSelect("SUM(CASE WHEN tx.purpose = 'product_purchase' THEN tx.amount ELSE 0 END)", "product")
      .addSelect("SUM(CASE WHEN tx.purpose = 'puja_confirmation' THEN tx.amount ELSE 0 END)", "puja")
