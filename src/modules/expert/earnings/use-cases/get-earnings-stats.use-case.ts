...
import { GetExpertPujasByDateUseCase } from '@/modules/puja-appointment/use-cases/get-expert-pujas-by-date.use-case';
...
export class GetEarningsStatsUseCase {
  constructor(
    ...
    private expertRepo: Repository<ExpertAccount>,
  ...
  async execute(expertProfileId: number, dto: GetExpertEarningsStatsDto) {
    ...
    const callRevenue = calls.reduce(
      (acc, c) => acc + Number(c.expert_earning || c.final_price || 0),
      0,
    );
    ...
    addDataToTrends(sessions as unknown as Record<string, unknown>[], (s) =>
      Number(s.expert_earning || s.total_cost || 0),
    );
    addDataToTrends(calls as unknown as Record<string, unknown>[], (c) =>
      Number(c.expert_earning || c.final_price || 0),
    );
    addDataToTrends(
