...
import {
  ...
  CouponStatus,
  CouponType,
} from '@/modules/commerce/coupon/entities/coupon.entity';
import { UserCoupon } from '@/modules/commerce/coupon/entities/user-coupon.entity';
...
export class CreateOrderFromCartUseCase {
  ...
  async execute(
    ...
    let platformSetting = await this.dataSource
      ...
      .findOne({ where: { key: 'PLATFORM_FEE' } });
    if (!platformSetting) {
      platformSetting = await this.dataSource
        .getRepository(SystemSetting)
        .findOne({
          where: {
            key: 'PLATFORM_FEE'.includes('COMMISSION')
              ? 'PLATFORM_FEE'.replace('COMMISSION', 'COMMISION')
              : 'PLATFORM_FEE'.replace('COMMISION', 'COMMISSION'),
          },
        });
    }
    const platformFee =
      platformSetting && platformSetting.value
    ...
    try {
      ...
      if (dto.coupon_code) {
        ...
        if (
          couponEntity.expiry_date &&
          new Date(couponEntity.expiry_date) < new Date()
        ) {
          throw new BadRequestException('This coupon has expired');
        }

        if (
          couponEntity.max_usage_limit &&
          couponEntity.usage_count >= couponEntity.max_usage_limit
        ) {
          throw new BadRequestException(
        ...
        if (
          couponEntity.min_order_value &&
          totalAmount < couponEntity.min_order_value
        ) {
          throw new BadRequestException(
            `Minimum order value of ₹${couponEntity.min_order_value} is required for this coupon`,
          );
        }
      ...
      this.logger.log(
        `[CREATE_ORDER] Total shipping calculated: ₹${totalShipping}`,
      );
      ...
      this.logger.log(
        `[CREATE_ORDER] Added platform fee ₹${platformFee}. Final totalAmount: ₹${totalAmount}`,
      );
      ...
      if (isWalletPayment) {
        ...
        try {
          ...
          this.logger.error(
            `[CREATE_ORDER] Failed to generate transaction no: ${(err as Error).message}`,
          );
        }
      ...
      const initialStatus = isWalletPayment
        ? OrderStatus.PAID
        : OrderStatus.PENDING;
      for (const item of itemsToCreate) {
