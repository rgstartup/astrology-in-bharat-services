      ...
      for (const item of items) {
        ...
        if (!groupedOrdersMap.has(orderIdStr)) {
          groupedOrdersMap.set(orderIdStr, {
            ...
            customerName:
              (
                item.order as unknown as {
                  client?: { user?: { name?: string }; name?: string };
                }
              )?.client?.name ||
              (
                item.order as unknown as {
            ...
            customerImage: (() => {
              const avatar =
                (item.order as any)?.client?.avatar ||
                (item.order as any)?.client?.user?.avatar ||
                null;
              console.log(
                'DEBUG AVATAR for Order',
                orderIdStr,
                ':',
                avatar,
                'Client:',
                (item.order as any)?.client,
              );
              return avatar;
            })(),
            shippingAddress: (item.order as any)?.shipping_address
              ? {
                  fullName:
                    (item.order as any).shipping_address.full_name ||
                    (item.order as any).shipping_address.fullName ||
                    '',
                  addressLine1:
                    (item.order as any).shipping_address.line1 ||
                    (item.order as any).shipping_address.addressLine1 ||
                    '',
                  addressLine2:
                    (item.order as any).shipping_address.line2 ||
                    (item.order as any).shipping_address.addressLine2 ||
                    '',
                  city: (item.order as any).shipping_address.city || '',
                  state: (item.order as any).shipping_address.state || '',
                  pincode:
                    (item.order as any).shipping_address.zip_code ||
                    (item.order as any).shipping_address.pincode ||
                    '',
                  phone: (item.order as any).shipping_address.phone || '',
                  alternatePhone:
                    (item.order as any).shipping_address.alternate_phone ||
                    (item.order as any).shipping_address.alternatePhone ||
                    '',
                }
              : null,
            amount: 0,
            ...
            date: item.order?.created_at
              ? new Date(item.order.created_at).toISOString()
              : new Date().toISOString(),
            shippingCharge: item.order?.shipping_charge
              ? Number(item.order.shipping_charge)
              : 0,
            itemsCount: 0,
