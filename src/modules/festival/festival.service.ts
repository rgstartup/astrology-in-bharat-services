    ...
    async findAll(year?: number, month?: number) {
        ...
        if (!festivals) {
            try {
                ...
                if (!response.ok) {
                    console.warn(`[FestivalService] API returned status: ${response.status}`);
                ...
                if (!yearData) {
                    console.warn(`[FestivalService] No data found for year ${targetYear}`);
                    festivals = [];
                } else {
                    const flattened: any[] = [];
                    // Loop through months
                    Object.keys(yearData).forEach(monthKey => {
                        ...
                        Object.keys(monthData).forEach(dateStr => {
                            ...
                            flattened.push({
                                ...
                                type: item.type?.toLowerCase().includes('holiday') ? 'holiday' : 'festival',
                                image_url: null
                            });
                        });
                    });
                ...
                console.log(`[FestivalService] Processed ${festivals.length} items from API`);
