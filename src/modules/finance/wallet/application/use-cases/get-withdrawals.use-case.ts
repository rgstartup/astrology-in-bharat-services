...
export class GetWithdrawalsUseCase {
  ...
  async execute(
    ...
    let ownerWhere: Record<string, unknown> = { id: null }; // Default fail if no owner found
    if (wallet.expert_id) ownerWhere = { expert_id: wallet.expert_id };
    else if (wallet.merchant_id)
      ownerWhere = { merchant_id: wallet.merchant_id };
    else if (wallet.agent_id)
      ownerWhere = { agent_profile_id: wallet.agent_id };
    ...
    console.log(
      `[GetWithdrawalsUseCase] First item details:`,
      items.length > 0
        ? {
            id: items[0].id,
            withdrawal_no: (items[0] as unknown as Record<string, unknown>)
              .withdrawal_no,
